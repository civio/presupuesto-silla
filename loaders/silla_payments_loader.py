...
class SillaPaymentsLoader(PaymentsLoader):
    ...
    def parse_item(self, budget, line):
        ...
        payee = re.sub(r'^B.Santander Central Hispa$', u'Banco Santander Central Hispano', payee)
        payee = re.sub(r'^B.Santander Central Hispano$', u'Banco Santander Central Hispano', payee)
        payee = re.sub(r'^Banco de Santander S\.A\.$', u'Banco Santander Central Hispano', payee)
        payee = re.sub(r'^Banco Santander C\.H\. S\.A\.$', u'Banco Santander Central Hispano', payee)
        payee = re.sub(r'^Banco Bilbao Vizcaya$', u'Banco Bilbao Vizcaya Argentaria', payee)
        payee = re.sub(r'^Banco Bilbao Vizcaya Argentaria \(Bcl\)$', u'Banco Bilbao Vizcaya Argentaria', payee)
        payee = re.sub(r'^Banc Sabadell$', u'Banco de Sabadell', payee)
        payee = re.sub(r'^Banco de Sabadell S\.A\.$', u'Banco de Sabadell', payee)
        payee = re.sub(r'^Banco Sabadell Urb 243$', u'Banco de Sabadell', payee)
        ...
        payee = re.sub(r'^C\.A\.M\.$', u'Caja de Ahorros del Mediterráneo', payee)
        payee = re.sub(r'^Caja de Rural Torrent$', u'Caja Rural de Torrent', payee)
        payee = re.sub(r'^Club Frontenis Incusa-Silla$', u'Club Frontenis Silla', payee)
        ...
        payee = re.sub(r'^Cobo de Medina Juan Dios$', u'Cobo Medina Juan de Dios', payee)
        payee = re.sub(r'^Comercial y Derivados Ele$', u'Comercial y Derivados Electricos', payee)
        payee = re.sub(r'^Comunidad Propie\.V\.Aleixa$', u'Comunidad de Propietarios Vicente Aleixandre', payee)
        payee = re.sub(r'^Comunidad Propietaris Vicente Aleixandre$', u'Comunidad de Propietarios Vicente Aleixandre', payee)
        ...
        payee = re.sub(r'^Didactic Sport S\.L\.$', u'Didactic Esport S.L.', payee)
        ...
        payee = re.sub(r'^Domenech Merin M\.Angeles$', u'Domenech Merin M Angeles', payee)
        payee = re.sub(r'^Encuadernaciones Ruiz$', u'Encuadernaciones Ruiz S.L.', payee)
        payee = re.sub(r'^Esquerra Unida del Pais V$', u'Esquerra Unida del Pais Valencià', payee)
        payee = re.sub(r'^Europea Servicio e Higien$', u'Europea de Servicio e Higiene S.A.', payee)
        payee = re.sub(r'^Europea Servicios Pub\.S\.A$', u'Europea de Servicio e Higiene S.A.', payee)
        payee = re.sub(r'^Farmacia Moliner Fernandez Luis V$', u'Farmacia Moliner Fernandez Luis V.', payee)
        payee = re.sub(r'^Farmacia Solanes Calatayud Inmacul$', u'Farmacia Solanes Calatayud Inmaculada', payee)
        payee = re.sub(r'^Farmacia Solanes Calatayud Maria I$', u'Farmacia Solanes Calatayud Inmaculada', payee)
        payee = re.sub(r'^Finsanzia Autorenting S\.A\.$', u'Finanzia Autorenting S.A.', payee)
        payee = re.sub(r'^Gas Natural Union Fenos\.A\.$', u'Gas Natural Union Fenosa S.A.', payee)
        payee = re.sub(r'^Gossos d\'Alpe S\.C\.$', u'Gossos Alpe S.C.', payee)
        payee = re.sub(r'^Els Verds$', u'Grup Els Verds de Silla', payee)
        payee = re.sub(r'^Iborra Ferradis Mayte$', u'Iborra Ferrandis Maria Teresa', payee)
        payee = re.sub(r'^Iborra Ferrandis Maria Teres\.A\.$', u'Iborra Ferrandis Maria Teresa', payee)
        payee = re.sub(r'^Innocarp$', u'Innocarp Stands S.L.', payee)
        payee = re.sub(ur'^Iranzo del Pontes Mª Pila$', u'Iranzo Pontes Mª Pilar', payee)
        payee = re.sub(ur'^Iranzo Pontes Mª Del Pila$', u'Iranzo Pontes Mª Pilar', payee)
        payee = re.sub(ur'^Jean François Alberghi$', u'Alberghi Jean François', payee)
        payee = re.sub(ur'^Jean-François Alberghi$', u'Alberghi Jean François', payee)
        ...
        payee = re.sub(r'^La Caixa A\. y Pens\.Barcel$', u'Caja de Ahorros y pensiones de Barcelona "La Caixa"', payee)
        payee = re.sub(r'^La Caixa A\. y Pens\.Barcelona$', u'Caja de Ahorros y pensiones de Barcelona "La Caixa"', payee)
        payee = re.sub(r'^Mapfre \(Seguros Generales$', u'Mapfre (Seguros Generales)', payee)
        payee = re.sub(r'^Mapfre Caucion y Credito$', u'Mapfre (Seguros Generales)', payee)
        payee = re.sub(r'^Mapfre Familiar S\.A\.$', u'Mapfre (Seguros Generales)', payee)
        payee = re.sub(r'^Mapfre S\.A\.$', u'Mapfre (Seguros Generales)', payee)
        payee = re.sub(r'^Mapfre Seguros de Empresas Cia de Seguros y Reaseguros S\.A$', u'Mapfre (Seguros Generales)', payee)
        payee = re.sub(r'^Mapfre Vida S\.A\.$', u'Mapfre (Seguros Generales)', payee)
        ...
        payee = re.sub(r'^Olivares Alfonso Teres\.A\.$', u'Olivares Alfonso Teresa (Imprimax)', payee)
        ...
        payee = re.sub(r'^Club Futbol Silla Atletic$', u'Silla Club de Fútbol', payee)
        ...
        payee = re.sub(r'^Trama de Gasllar S\.A\.$', u'Trama de Gasllar S.L.', payee)
        payee = re.sub(r'^Viversilla Coop\.Val\.$', u'Viver - Silla Coop. Valenciana', payee)
        payee = re.sub(r'^Viversilla Planta Ornamental S\.L\.$', u'Viversilla Planta Ornamental S.L.L.', payee)
        ...
        payee = re.sub(r'^Smax Sl \(Serv\.Medioamb\.Xilxes\)$', u'Servicios Medioambientales Xilxes S.L.', payee)
        payee = re.sub(r'Infraestruc Sau-', u'Infraestructuras S.A.U. - ', payee)
        payee = re.sub(r'^Enti\.Metropolitana T\.Res\.$', u'Entidad Metropolitana para el Tratamiento de Residuos', payee)
        payee = re.sub(r'Felisa\(Decojar\)', u'Felisa (Decojar)', payee)
        payee = re.sub(r'Comerc\.Ultimo', u'Comercial Ultimo', payee)
        payee = re.sub(r'^Objetivo Const\.Proyect\.S\.L\.$', u'Objetivo de Construcciones y Proyectos S.L.', payee)
        payee = re.sub(r'^Nominas en Formalizacion$', u'Nóminas en formalización', payee)
        payee = re.sub(r'Col\.Legi', u'Col.legi', payee)
        payee = re.sub(r'Col\.Lectiu', u'Col.lectiu', payee)
        payee = re.sub(r'^Ions\.A\.$', u'IONSA', payee)
        payee = re.sub(r'^UTE EIFFAGE INFra\.ESTRUC SAU-EIFFASE ENERGIA S\.L\.U\.$', u'UTE Eiffage Infraestructuras S.A.U. - Eiffase Energia S.L.U.', payee)
        payee = re.sub(r'Construc\.Machancoses', u'Construcciones Machancoses', payee)
